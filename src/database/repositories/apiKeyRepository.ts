...
export const apiKeyRepository = {
  ...
  async saveApiKey(params: {
    ...
    // Keep legacy settings table in sync for backward compatibility
    if (shouldBeActive) {
      await db.runAsync(
        `INSERT INTO settings (key, value, updated_at) VALUES ('gemini_api_key', ?, ?)
         ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at;`,
        params.apiKey.trim(),
        now
      );
      await db.runAsync(
        `INSERT INTO settings (key, value, updated_at) VALUES ('gemini_model', ?, ?)
         ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at;`,
        defaultModel.trim(),
        now
      );
